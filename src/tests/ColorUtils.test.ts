...
describe("ColorUtils.getChordColor", () => {
  describe("interval and inverse resolve to the same color", () => {
    it("P4 and P5 (IC 5) produce the same color", () => {
      ...
      const colorP4 = ColorUtils.getChordColor(ixActualArray([0, 5]));
      const colorP5 = ColorUtils.getChordColor(ixActualArray([0, 7]));
      expect(colorP4).toBe(colorP5);
    });

    it("m2 and M7 (IC 1) produce the same color", () => {
      // [0, 1] = m2, [0, 11] = M7 — both interval class 1
      const colorM2 = ColorUtils.getChordColor(ixActualArray([0, 1]));
      const colorM7 = ColorUtils.getChordColor(ixActualArray([0, 11]));
  ...
  describe("triads", () => {
    it("color(sus2) = color(sus4)", () => {
      ...
      const colorSus2 = ColorUtils.getChordColor(ixActualArray([0, 2, 7]));
      const colorSus4 = ColorUtils.getChordColor(ixActualArray([0, 5, 7]));
    ...
    it("color(major) = color(major 1st inversion), e.g. color(C) = color(C/E)", () => {
      ...
      const colorRoot = ColorUtils.getChordColor(ixActualArray([0, 4, 7]));
      const colorFirstInv = ColorUtils.getChordColor(ixActualArray([4, 7, 12]));
    ...
    it("color(minor) ≠ color(major)", () => {
      ...
      const colorMajor = ColorUtils.getChordColor(ixActualArray([0, 4, 7]));
      const colorMinor = ColorUtils.getChordColor(ixActualArray([0, 3, 7]));
  ...
  describe("tetrachords", () => {
    it("color(ø7)=color(min6)", () => {
      ...
      const colorHalfDiminished7 = ColorUtils.getChordColor(
        ixActualArray([0, 3, 6, 10]),
      );
      const colorMin6 = ColorUtils.getChordColor(ixActualArray([0, 3, 7, 9]));
    ...
    it("color(min7)=color(maj6)", () => {
      ...
      const colorMin7 = ColorUtils.getChordColor(ixActualArray([0, 3, 7, 10]));
      const colorMaj6 = ColorUtils.getChordColor(ixActualArray([0, 4, 7, 9]));
    ...
    it("color of Maj7 is distinguishable from +Maj7", () => {
      ...
      const colorMaj7 = ColorUtils.getChordColor(ixActualArray([0, 4, 7, 11]));
      const colorPlusMaj7 = ColorUtils.getChordColor(
        ixActualArray([0, 4, 8, 11]),
      );
